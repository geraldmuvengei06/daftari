...
export default function LoginPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!otp.trim()) { setErrors({ otp: "Enter the code" }); return }
    setLoading(true)
    setErrors({})
    ...
    if (error) { setLoading(false); setErrors({ otp: error.message }); return }
    try { await ensureTenant(phone) } catch {}
    router.push("/customers")
  }
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4" noValidate>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="123456"
                  value={otp}
                  onChange={(e) => { setOtp(e.target.value); setErrors({ ...errors, otp: undefined }) }}
              ...
              <Button type="button" variant="ghost" className="w-full" onClick={() => { setSent(null); setOtp(""); setErrors({}) }}>
                Try again
              </Button>
            </form>
          </CardContent>
      ...
      <div className="flex min-h-svh items-center justify-center px-4">
        <Card className="w-full max-w-sm">
          ...
          <CardContent className="text-center">
            <Button variant="ghost" className="w-full" onClick={() => { setSent(null); setErrors({}) }}>
              Try again
            </Button>
          </CardContent>
        </Card>
          ...
          <Tabs defaultValue="email" onValueChange={() => setErrors({})}>
            ...
            <TabsContent value="phone">
              <form onSubmit={handlePhone} className="space-y-4 pt-2" noValidate>
                <p className="text-xs text-muted-foreground">Phone login requires an SMS provider. Use email for now.</p>
                <div className="space-y-2">
